...
# 输出文件的绝对路径
file_path = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'evaluate_templates', 'mapping_template.j2')
print(file_path)
...
if 'DEBUG_MODE' in os.environ:
    debugpy.listen(5678)  # 设置监听的端口号，可以根据需要修改
    print("Waiting for debugger attach...")
    debugpy.wait_for_client()
    print("Debugger attached. Resume execution now.")
...
c, c_factors = find_max_factors_in_range(C_SIZE)         #c_factors是c的质因数个数
...
if __name__ == "__main__":
    ...
    tensor = np.random.randint(low=0, high=100, size=63, dtype=np.int)
    tensor[ :7] = np.random.permutation(7)
    tensor[7:14] = np.random.permutation(7)
    tensor[14:21] = np.random.permutation(7)
    tensor[21] = np.random.randint(low=0, high=c_bound)
    tensor[22] = np.random.randint(low=0, high=m_bound)
    tensor[23] = np.random.randint(low=0, high=n_bound)
    tensor[24] = np.random.randint(low=0, high=p_bound)
    tensor[25] = np.random.randint(low=0, high=q_bound)
    tensor[26] = np.random.randint(low=0, high=r_bound)
    tensor[27] = np.random.randint(low=0, high=s_bound)
    tensor[28] = np.random.randint(low=0, high=8)            #split位置
    ...
    for j in range(Input_UOP_free,47):
        tensor[j] =  np.random.randint(low=1, high=4)  

    tensor[47:59] = 0
    for j in range(Weight_UOP_free,59):
        tensor[j] =  np.random.randint(low=1, high=4)

    tensor[59] = np.random.randint(low=0, high=8)                   #GLB  I/W  S/G
    tensor[60] = np.random.randint(low=0, high=8)                   #GLB   O   S/G
    tensor[61] = np.random.randint(low=0, high=8)                   #PEB  I/W  S/G
    tensor[62] = np.random.randint(low=0, high=8)                   #PEB   O   S/G

    s = evaluate_individual(tensor)
    print(s)
