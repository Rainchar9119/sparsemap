...
WORK_LOAD = "workload_deepbrain_DSEsample"           #低层次
#PLATFORM = "cloud"
PLATFORM = "edge"
#PLATFORM = "mobile"
N_GENERATIONS = 10
...
DSE_plot_path = os.path.join(pictures_dir, "DSE_SAMPLE.png")
...
def DSE_SPACE():
    ...
    performance = []
    sampled_indis_sp_opt = []
    sampled_indis_mapping = []
    points = []

    for j in range(POINT_SIZE):
        indi = np.array([0 for m in range(DNA_SIZE)])
        for i in range(15):
            indi[i] = random.randint(1,3)
        for i in range(15,19):
            indi[i] = random.randint(0,7)
        for i in range(19,21):
            indi[i] = random.randint(1,2)
        for i in range(21,23):
            indi[i] = random.randint(0,7)
        for i in range(23,28):
            indi[i] = random.randint(1,6)
        for i in range(28,DNA_SIZE):
            indi[i] = random.randint(0,4)
        sampled_indis_sp_opt.append(indi[0:19])
        sampled_indis_mapping.append(indi[19:DNA_SIZE])
        performance.append(- evaluate(indi))
    
    np_sp_opt = np.array(sampled_indis_sp_opt)
    np_mapping = np.array(sampled_indis_mapping)

    pca = PCA(n_components=1)
    x = pca.fit_transform(np_mapping)
    y = pca.fit_transform(np_sp_opt)
    np_performance = np.array(performance)
    # 创建一个掩码来标记无穷大值的位置
    mask = np.isinf(np_performance)
    # 将无穷大值替换为最大有限值，用于颜色映射
    finite_values = np.copy(np_performance)
    finite_values[mask] = np.nan  # 暂时设置为NaN，后面颜色映射时再处理


    colors = ["darkred", "red", "orange", "indigo", "mediumvioletred"]
    new_cmap = ListedColormap(colors)

    plt.figure(figsize=(8, 6))
    plt.scatter(x[mask], y[mask], color='lightgray', s=2, alpha=0.7)
    scatter = plt.scatter(x[~mask], y[~mask], c=finite_values[~mask], cmap=new_cmap, s=2, alpha=0.7)

    cbar = plt.colorbar(scatter, label='Values')
    cbar.cmap.set_bad(color='lightgray')
...
def DSE_SPACE_new():
    ...
    for j in range(POINT_SIZE):
        ...
        for i in range(28,DNA_SIZE):
            indi[i] = random.randint(0,4)
        sampled_indis_sp_opt.append(indi[0:19])                       #原来是15
        sampled_indis_mapping.append(indi[19:DNA_SIZE])
        performance.append(- evaluate(indi))
    ...
    # 使用渐变颜色映射，例如 'viridis', 'plasma', 'inferno', 'magma' 等
    ...
    plt.scatter(x[mask], y[mask], color='lightgray', s=2, alpha=0.7)
    # 绘制非无穷大值的点，使用渐变颜色
    scatter = plt.scatter(x[~mask], y[~mask], c=finite_values[~mask], cmap=cmap, s=2, alpha=0.7)

    # 添加颜色栏
    cbar = plt.colorbar(scatter, label='Performance')
    cbar.cmap.set_bad(color='lightgray')
